...
while cap.isOpened():
    ...
    cnt = sorted(cv2.findContours(canny, mode=cv2.RETR_CCOMP, method=cv2.CHAIN_APPROX_SIMPLE)[-2], key=cv2.contourArea)[-1]
...
# # edges = cv2.erode(canny, kernel, iterations = 1)
# edges = cv2.dilate(canny, kernel_dil, iterations = 1)
# cnt = sorted(cv2.findContours(edges, mode=cv2.RETR_TREE, method=cv2.CHAIN_APPROX_SIMPLE)[-2], key=cv2.contourArea)[-1]
# # contours, _ = cv2.findContours(edges, mode=cv2.RETR_TREE, method=cv2.CHAIN_APPROX_SIMPLE)
# mask = np.zeros((h, w), np.uint8)
# masked = cv2.drawContours(mask, [cnt], -1, 255, -1)
